    ...
    <div class="list-wrapper">
        <div class="row card-wrapper">
            <div v-for="(user, index) in userData" :key="index" class="col-md-4 mb-3 ">
                <b-card class="pointer" @click="handleLocation(user.location.coordinates)" :img-src="user.picture.large"
                    ...
                    <b-card-text class="m-h">
                        ...
                        <p class="pointer" @click="handleLocation(user.location.coordinates)"><strong>country:</strong> {{
                            user.location.country }}</p>
                        <p class="pointer" @click="handleLocation(user.location.coordinates)"><strong>Gender:</strong> {{
                            user.gender }}</p>
                        <p class="pointer" @click="handleLocation(user.location.coordinates)"><strong>Email:</strong> {{
                            user.email }}</p>
                        <p class="pointer" @click="handleLocation(user.location.coordinates)"><strong>DOP:</strong>
                            {{ formatDate(user.dob.date) }}</p>
                    </b-card-text>
                </b-card>
            </div>
        </div>
        <div class="btn-wrapper">
            <b-button variant="primary" class="more-btn" v-if="userData.length" @click="handleNext">NEXT</b-button>
        </div>
...
<script>
import getData from "../services/list.service"
...
export default {
    ...
    methods: {
        async query() {
            ...
            try {
                const promises = Array.from({ length: 5 }, () => getData.get());
